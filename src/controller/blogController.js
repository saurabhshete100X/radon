...
const createBlog = async function (req, res) {
    try {
        ...
        if (!details.title) return res.status(400).send({
            status: false,
            msg: "Title of the blog is required"
        });
        if (!details.body) return res.status(400).send({
            status: false,
            msg: "Body of the blog is required"
        });
        if (!details.authorId) return res.status(400).send({
            status: false,
            msg: "Author_Id of the blog is required"
        });
        if (!details.category) return res.status(400).send({
            status: false,
            msg: "Category of the blog is required"
        });
        const validate = await authorModel.findById(details.authorId)
        if (!validate) return res.status(400).send({
            status: false,
            msg: "Please enter valid authorId"
        })
...
const getBlog = async function (req, res) {
    try {
        ...
        let getData = await BlogModel.findOne({
            authorId: valid,
            tags: tags,
            category: category,
            isDeleted: false,
        ...
        if (!getData) return res.status(404).send({
            status: false,
...
const updateBlog = async function (req, res) {
    try {
        ...
        if (!checkBlog) return res.status(404).send({
            status: false,
            msg: "Blog Not Found"
        })
        if (checkBlog.isDeleted == true) return res.status(400).send({
            status: false,
